...
overlapList =[]
while overlap < 20:
    for overlap_search1 in range(20):
        for overlap_search2 in range(20):
            if overlap_search1 == overlap_search2:
                pass
            elif randList[overlap_search1] == randList[overlap_search2]:
                count+=1
                if randList[overlap_search2] not in overlapList:
                    overlapList.append(randList[overlap_search2])
        overlap +=1
        if count==1:
            continue
        else:
            for counting1 in range(len(overlapList)):
                for counting2 in range(len(overlapList)):
                    if counting1 == counting2:
                        continue
                    elif overlapList[counting1] == overlapList[counting2]:
                        del overlapList[counting1]
        count = 1
play = 1
while play < len(overlapList):
    print(overlapList[play-1])
    play+=1
